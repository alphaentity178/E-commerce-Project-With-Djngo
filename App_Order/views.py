    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity >= 1 :
                ...
                order_item.save()
                messages.info(request , f'You have increased the quantity of {item.name} to {order_item.quantity}')
                return redirect('App_Order:cart')
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity >= 1 :
                order_item.quantity -= 1
                order_item.save()
                messages.info(request , f'You have decreased the quantity of {item.name} to {order_item.quantity}')
                return redirect('App_Order:cart')
            else :
                order.orderitems.remove(order_item)
                order_item.delete()
                messages.warning(request , 'You can\'t decrease the quantity below 1. This item has been removed from your cart.')
                return redirect('App_Shop:home')
